...
OFE_BINS = np.arange(0., 0.51, 0.005).tolist() 
XLIM = [-0.02, 0.52] 
YLIM = [0.005, 0.5] 
FEH_BINS = [ 
	[-0.8, -0.7], 
...
def plot_pdfs(ax, stars, min_rgal, max_rgal, label = False): 
	for i in range(len(FEH_BINS)): 
		...
		ax.plot(
			...
			get_ofe_pdf(stars, min_rgal, max_rgal, FEH_BINS[i][0], FEH_BINS[i][1]), 
			**kwargs) 
...
if __name__ == "__main__": 
	...
	plot_pdfs(axes[0], out.tracers, 3, 5) 
	plot_pdfs(axes[1], out.tracers, 5, 7) 
	plot_pdfs(axes[2], out.tracers, 7, 9) 
	plot_pdfs(axes[3], out.tracers, 9, 11) 
	plot_pdfs(axes[4], out.tracers, 11, 13, label = True) 
